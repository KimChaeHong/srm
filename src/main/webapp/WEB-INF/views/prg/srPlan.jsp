...
<form id="sr-plan-form" action="updateSrPlan" method="post">
	...
	<c:choose>
        <c:when test="${memInfo != null && memInfo.role1 == 'ROLE_DEVE'}">
        	<!-- 개발자인 경우 -->
		...
		<c:otherwise>
			...
			<div class="form-group">
	                ...
	                <select id="request-type" class="form-select" name="srType" disabled>
	                    <option value="IMPR" ${srPlan.srType == 'IMPR' ? 'selected' : ''}>개선</option>
	                    <option value="NEW" ${srPlan.srType == 'NEW' ? 'selected' : ''}>신규</option>
	                    <option value="ERRO" ${srPlan.srType == 'ERRO' ? 'selected' : ''}>오류</option>
	                    <option value="OTHR" ${srPlan.srType == 'OTHR' ? 'selected' : ''}>기타</option>
	            ...
	            <div class="form-group">
	                <label for="team">처리팀</label>
	                <select id="team" name="deptId" class="form-select" disabled>
	                    <option value="DEV1" ${srPlan.deptId == 'DEV1' ? 'selected' : ''}>개발 1팀</option>
	                    <option value="DEV2" ${srPlan.deptId == 'DEV2' ? 'selected' : ''}>개발 2팀</option>
	                </select>
	            </div>
	            <div class="form-group">
	                ...
	                <select id="work-type" class="form-select" name="wkType" disabled>
	                    <!-- 작업 유형 옵션들 동일하게 추가 -->
	                </select>
	            </div>
	            <div id="person-group" class="form-group">
	                <label for="person" id="dam">담당자</label>
	                <div class="input-button-group">
	                    <input type="text" id="person" value="${srPlan.mgr}" disabled style="width: 130px;">
	                    <!-- <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#mgr-modal" disabled>추가</button> -->
	                </div>
	            </div>
	            <div class="form-group">
	                <label for="start-date">목표 시작일</label>
	                <input type="date" id="start-date" name="trgStDt" value="<fmt:formatDate value='${srPlan.trgStDt}' pattern='yyyy-MM-dd'/>" disabled>
	            </div>
	            <div class="form-group">
	                <label for="end-date">목표 완료일</label>
	                <input type="date" id="start-date" name="trgEndDt" value="<fmt:formatDate value='${srPlan.trgEndDt}' pattern='yyyy-MM-dd'/>" disabled>
	            </div>
	            <div class="form-group">
	                <label for="status">접수 상태</label>
	                <select id="status" class="form-select" name="rcpStat" disabled>
	                    <option value="DECE" ${srPlan.rcpStat == 'DECE' ? 'selected' : ''}>접수</option>
	                    <option value="CANC" ${srPlan.rcpStat == 'CANC' ? 'selected' : ''}>취소</option>
	                    <option value="HOLD" ${srPlan.rcpStat == 'HOLD' ? 'selected' : ''}>보류</option>
	                </select>
	            ...
	            <div class="form-group">
	                <label for="review">검토 내용</label>
	                <textarea id="review" name="rvwCont" disabled>${srPlan.rvwCont }</textarea>
	            </div>
        </c:otherwise>
    </c:choose>
</form>
